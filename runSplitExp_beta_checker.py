...
# m_split, n_split, its_split, m_notsplit, n_notsplit, its_notsplit = system.run(sample = 55, save = False)
...
inputs_sys_notsplit['M'] = 3*kwargs['M']
inputs_sys_notsplit['rho'] = kwargs['rho']/3
...
print(np.array_equal(m1_split, m_split[sample,beta_idx]))
print(np.array_equal(n1_split, n_split[sample,beta_idx]))
print(np.array_equal(its1_split, its_split[sample,beta_idx]))
print(np.array_equal(m1_notsplit, m_notsplit[sample,beta_idx]))
print(np.array_equal(n1_notsplit, n_notsplit[sample,beta_idx]))
print(np.array_equal(its1_notsplit, its_notsplit[sample,beta_idx]))
...
output = sb(entropy = entropy, checkers = (checker0, notsplit.J, rng_seeds_notsplit), disable = False, **kwargs_new)

m2_split, n2_split, its2_split, m2_notsplit, n2_notsplit, its2_notsplit = splitting_beta(entropy = entropy, **kwargs)
